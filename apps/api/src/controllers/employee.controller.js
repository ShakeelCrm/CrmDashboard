import { loginEmployee, createEmployee as createEmployeeService, getEmployeeByEmail, getEmployeeById, refreshAccessToken, logoutEmployee, canEmployeeLogin } from "../services/employee.service";
import { comparePassword } from "../utils/password.util";
import { ValidationError, AuthenticationError, ConflictError, NotFoundError, } from "../utils/error.util";
...
export const loginEmployeeController = async (req, res, next) => {
    try {
        ...
        if (!canLogin) {
            throw new AuthenticationError("Employee is already logged in from another device. Please log out from other devices first.");
        }
...
// @access  Public (uses refresh token)
export const logoutEmployeeController = async (req, res, next) => {
    try {
